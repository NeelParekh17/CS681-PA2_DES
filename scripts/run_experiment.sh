...
if [[ $# -gt 2 ]]; then
  ...
  if [[ -z "$inferred_config" ]]; then
    ...
    echo "Usage: $0 <config.properties> [out/summary.csv]" >&2
    echo "Example: $0 \"$ROOT/configs/experiment.properties\" \"$ROOT/out/summary.csv\"" >&2
    exit 2
  fi
...
mkdir -p "$(dirname "$OUT")"
java -cp build/classes des.Main --mode experiment --config "$CONFIG" --out "$OUT"
